...
/** Bannière EmeriaMC 600×120 (bas de l'embed). */
...
/** Petit logo EmeriaMC (coin haut-gauche de l'embed). */
const LOGO = '';

/** Petit embed d'annonce de reset (violet, large, logo en haut-gauche, date/heure). */
export function resetEmbed(title: string, body: string): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(VIOLET)
    .setAuthor({ name: 'EmeriaMC', iconURL: 'attachment://logo.png' })
    .setDescription(
      `# ${title}\n` +
        `-# ➜ <@&${RESET_ROLE}>\n\n` +
        `${body}\n` +
 ...
 * un nouveau reset du même type supprime l'ancien. Ping du rôle Reset Monde.
 */
export async function postReset(client: Client, embed: EmbedBuilder, type: 'end' | 'monde'): Promise<void> {
  ...
  const files = [
    ...
    new AttachmentBuilder(Buffer.from(LOGO, 'base64'), { name: 'logo.png' }),
  ];
